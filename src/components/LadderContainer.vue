<template>
  <div
    class="grow overflow-y-auto overflow-x-auto text-xxs pb-8 fixed top-12 left-80 bottom-0 l-2 bg-white"
  >
  <!-- <div class="absolute sticky top-0 w-full h-12 flex justify-center z-50 left-0 right-0">
    ...
    <div class="absolute sticky   justify-start z-50 flex">
      <div class="flex border-r border-r-slate-500 border-l border-l-slate-500">
        <div class="" v-if="Object.keys(deltaLadderFrom).length > 0">
          <div v-for="(item, index) in ladder" :key="index" class="">
            <div v-if="item['filled']" class="h-4  text-center">
              <div class="border-b border-b-white border-t border-t-white w-10 px-2" :class="getCumDeltaColor(getCumDelta(item))">{{ getCumDelta(item)}}</div>
            </div>
          </div>
        </div>
        <div class="">
        ...
        <div class="">
          <div v-for="(item, index) in ladder" :key="index" class="">
            <div v-if="item['filled']" class="h-4  text-center ">
              <div class="border-b border-b-white border-t border-t-white w-10 px-2" :class="getCumDeltaColor(getCumDelta(item, false))">{{ getCumDelta(item, false)}}</div>
            </div>
          </div>      
        </div>
      </div>
...
export default {
  ...
  setup() {
    ...
    const getCumDelta =(item, from=true) => {
      ...
      const res = from ? Math.round(deltaLadderFrom.data[item.from.at(-1).price.toFixed(2)].cum_delta) : Math.round(deltaLadderTo.data[item.to.at(0).price.toFixed(2)].cum_delta)
      if (px) {
